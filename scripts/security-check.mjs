 ...
 *
 * `credential.mjs` reads the environment, holds the stored sign-in, renews it, and words every message
 * about it. The failure this prevents is the ordinary one: a second door added later that reads the
 * environment directly or opens the store file itself, and so never learns about the stored sign-in,
 * sends the author back to a browser that has already approved it, or keeps working from a credential
 * the others have forgotten.
 *
 * THREE THINGS, not one. The first version stated a rule about the Hub address in its comment and had no
 * pattern behind it, and there were two copies of that address in the tree while it printed "ok" —
 * exactly the shape this repo calls a bug with a delay on it. Every rule below is a pattern that runs.
 *
 * The env rule matches the three spellings somebody writes without trying to evade anything. It is not
 * proof against a determined author; nothing a regex does is. It catches the honest second door.
 */
{
  const ALLOWED = new Map([
    ["scripts/credential.mjs", "owns all three"],
    /* The check that PROVES the owner: it clears the variable and reads the store to test them. */
  ...
  const walk = (dir) => {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      const full = join(dir, e.name);
      if (e.isDirectory()) walk(full);
      else if (/\.(js|mjs|cjs|ts|mts|md|json|sh)$/.test(e.name)) files.push(full);
    }
  };
  walk(ROOT);
  const strays = [];
  for (const f of files) {
    ...
    for (const [pattern, what] of RULES)
      if (pattern.test(text)) strays.push(`${relative} ${what}`);
  }
  probe(
    strays.length === 0,
    `only ${[...ALLOWED.keys()][0]} decides what a credential is, where one is made, and where it is kept`,
