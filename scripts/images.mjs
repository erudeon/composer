 ...
 * An HTML body here means something other than the API answered: a login page, a proxy, or a gateway.
   ...
   *
   * The route answers 200, 207 or 422 with `uploaded` and `failed`. Anything else — a 500, a 413, a 400,
   * an auth refusal — carries neither array, so pushing them appends nothing and the run reports fewer
   * figures than it was given WITHOUT reporting a single failure. On a course big enough to need two
   * requests that is the whole of the second batch going missing under a success message, and the map
   * file below would then be substituted into a manifest with those markers pointing at nothing.
   */
